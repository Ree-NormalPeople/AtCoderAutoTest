...
その他Chrome、Firefoxベースのブラウザでも動作が期待されます。Floorpでの動作は確認しました。
---
...
## 注意点
- インタラクティブ形式の問題は**対応していません**。インタラクティブ形式の問題でテストを実行すると、`入力例が見つかりませんでした。` と表示されます。その他特殊な問題については対応していない可能性があります。
- paiza.IOの `guest` APIキーを使っているため、混雑時は遅くなることがあります。また、paiza.IOの二秒間の計算量はだいたい10^7回程度（python）です。様々な仕様の相違により、AtCoderでACのコードでもpaiza.IOではTLEになることがあります。
...
- AtCoderの問題ページのhtml構造が変わると動作しなくなる可能性があります。もし動かなくなった場合は、issueを立てていただけると助かります。
---
